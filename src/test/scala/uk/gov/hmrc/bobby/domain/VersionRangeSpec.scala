...
class VersionRangeSpec extends AnyWordSpec with Matchers {

  "A VersionRange" should {
    ...
    "throw an IllegalArgumentException when incomplete version is provided" in {
      an[IllegalArgumentException] should be thrownBy VersionRange("[1.5,)")
    }

    "throw an IllegalArgumentException when brackets are missing" in {
      an[IllegalArgumentException] should be thrownBy VersionRange("1.5,)")
      an[IllegalArgumentException] should be thrownBy VersionRange("[1.5,")
      an[IllegalArgumentException] should be thrownBy VersionRange("1.5")
    }

    "throw an IllegalArgumentException when the range is open on both sides" in {
      an[IllegalArgumentException] should be thrownBy VersionRange("(,1.5,)")
    }

    "throw an IllegalArgumentException when multiple sets are used" in {
      an[IllegalArgumentException] should be thrownBy VersionRange("(,1.0],[1.2,)")
    }

    "include 1.2.5 when the expression is [1.2.0,1.3.0]" in {
      VersionRange("[1.2.0,1.3.0]").includes(Version("1.2.5")) shouldBe true
    }

    "include 0.2.0 when the expression is (,1.0.0]" in {
      VersionRange("(,1.0.0]").includes(Version("0.2.0")) shouldBe true
    }

    "include 1.2.0 when the expression is [1.0.0,)" in {
      VersionRange("[1.0.0,)").includes(Version("1.2.0")) shouldBe true
    }

    "not include the left boundary when the expression is (1.0.0,)" in {
      VersionRange("(1.0.0,)").includes(Version("1.0.0")) shouldBe false
    }

    "not include the right boundary when the expression is (,1.0.0)" in {
      VersionRange("(,1.0.0)").includes(Version("1.0.0")) shouldBe false
    }

    "include snapshots when the version is 1.0.0-SNAPSHOT" in {
      VersionRange("[*-SNAPSHOT]").includes(Version("1.0.0-SNAPSHOT")) shouldBe true
    }

    "not include snapshots when the version is 1.0.0" in {
      VersionRange("[*-SNAPSHOT]").includes(Version("1.0.0")) shouldBe false
    }

    "build string that contains inclusive lower-bound and upper-bound" in {
      VersionRange("[1.2.0,1.3.0]").toString shouldBe "[1.2.0,1.3.0]"
    }

    "build string that contains wildcard" in {
      VersionRange("[*-SNAPSHOT]").toString shouldBe "[*-SNAPSHOT]"
    }

    "build string that contains non-inclusive lower-bound and upper-bound" in {
      VersionRange("(1.2.0,1.3.0)").toString shouldBe "(1.2.0,1.3.0)"
    }

    "understand play cross compiled libraries and ignore play suffixes" in {
    ...
    "throw exception when qualifier is not defined" in {
